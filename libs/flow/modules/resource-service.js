	...
	function projectResourceFile(path, mustExist, env) {
		...
		if (!env.isAllowedResourcePath(normalized)) {
			env.raise("RESOURCE_PATH_NOT_ALLOWED", "Flow resource path is not editable through this API: " + normalized,
				null, "Allowed paths: libs/flow/engine.yaml, libs/flow/blocks/**/*.block.js, libs/flow/blocks/**/*.hooks.js, libs/flow/lib/**/*.js, libs/flow/resources/**/*.{md,txt,json,yaml,yml}, libs/flow/frontbuilder/**/*.{flow.svelte,front.json,uiblock.json}, libs/flow/types/**/*.{type.yaml,js}, libs/flow/types/editors/**/*.{html,css,js}.");
		}

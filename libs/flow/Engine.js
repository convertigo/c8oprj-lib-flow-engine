(function () {
	...
	function propertyEditorHtml() {
		return "<!doctype html><html><head><meta charset=\"utf-8\">"
			...
			+ ".modebar,.copybar{display:flex;gap:6px;align-items:center;margin:6px 0}.modebar button.active{background:#0d9ac2;border-color:#2bb8df}.simple{display:grid;grid-template-columns:1fr;gap:8px;margin-bottom:6px}.simple label{display:block;margin:0}.simpleLabel{display:block;margin-bottom:3px;color:#9da7af;font-weight:600}.simple input[readonly]{background:#171a1d;color:#b8dff4}.hidden{display:none!important}.copybar input{flex:1}.copybar select{width:auto;min-width:90px;border:1px solid #48525c;background:#111417;color:#f2f6f8;border-radius:4px;padding:5px;font-size:12px}"
			...
			+ "function simpleParts(v){v=String(v||'');var m=v.match(/^\\s*\\{\\{([\\s\\S]*)\\}\\}\\s*$/);if(m)return{prefix:'',path:m[1].trim(),suffix:''};return{prefix:'',path:'',suffix:''};}"
			+ "function simpleCandidate(v){return String(v||'').trim()===''||simpleParts(v).path!=='';}"
			+ "function simpleValue(){var p=document.querySelector('[data-simple=\"prefix\"]');var m=document.querySelector('[data-simple=\"pick\"]');var s=document.querySelector('[data-simple=\"suffix\"]');var path=m?m.value.trim():'';var expr=((p?p.value:'')+path+(s?s.value:'')).trim();return expr?'{{ '+expr+' }}':'';}"
			...
			+ "function propertyField(){var def=state.propertyDefinition||{};var key=state.property||'';var kind=def.kind||def.editor||'text';var value=state.value==null?'':String(state.value);var label=def.label||key||'value';var html='<div class=\"field\"><label>'+esc(label)+' <span class=\"kind\">'+esc(propertyTypeLabel(def))+'</span></label>';if(def.description||def.shortDescription)html+='<div class=\"desc\">'+esc(def.description||def.shortDescription)+'</div>';if(hasTypeEditor(kind)){var tag=typeEditorTag(kind);return html+'<'+tag+' data-key=\"'+esc(key)+'\" data-kind=\"'+esc(kind)+'\"></'+tag+'></div>';}var simple=templateLike(kind);if(simple)editorMode=simpleCandidate(value)?'simple':'custom';if(simple){var p=simpleParts(value);html+='<div class=\"modebar\"><button data-editor-mode=\"simple\" class=\"'+(editorMode==='simple'?'active':'')+'\">Simple</button><button data-editor-mode=\"custom\" class=\"secondary '+(editorMode==='custom'?'active':'')+'\">Custom</button></div>';html+='<div data-simple-box class=\"simple '+(editorMode==='simple'?'':'hidden')+'\"><label><span class=\"simpleLabel\">Prefix</span><input data-simple=\"prefix\" placeholder=\"expression prefix\" value=\"'+esc(p.prefix)+'\"></label><label><span class=\"simpleLabel\">Pick</span><input data-simple=\"pick\" placeholder=\"select a scope path\" value=\"'+esc(p.path)+'\" readonly></label><label><span class=\"simpleLabel\">Suffix</span><input data-simple=\"suffix\" placeholder=\"expression suffix\" value=\"'+esc(p.suffix)+'\"></label></div><div class=\"desc\">Prefix, pick and suffix are concatenated inside {{ expression }}.</div>';}html+='<textarea data-key=\"'+esc(key)+'\" data-kind=\"'+esc(kind)+'\" class=\"'+(simple&&editorMode==='simple'?'hidden':'')+'\">'+esc(value)+'</textarea>';return html+'</div>';}"
			...
			+ "function attachTypeEditor(){var tag=typeEditorTag(currentPropertyKind());var editor=document.querySelector(tag+'[data-key]');if(editor&&editor.setState){window.flowHost={request:hostRequest,setValue:setDraft};editor.flowHost=window.flowHost;editor.setState(state);focusKey=editor.getAttribute('data-key');editor.addEventListener('flow-value',function(e){setDraft(e.detail&&e.detail.value);});editor.addEventListener('flow-values',function(e){var d=e.detail||{};if(d.value!==undefined)setDraft(d.value);send({type:'values',value:d.value,values:d.values||{}});});setDraft(editor.value||'');return true;}return false;}"

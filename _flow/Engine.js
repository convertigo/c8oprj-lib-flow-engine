(function () {
	...
	function engineDir() {
		...
		return new File("_flow").getAbsoluteFile();
	}
	...
	function describeTreeCacheFamilyKey(request) {
		...
		var options = [
			String(request.sourceWritable !== false),
	...
	function describeFrontendDocument(request) {
		...
		// Every source layout uses the provider's model and descriptor contract.
		// Speed comes from the document caches and persistent provider, not a second AST projection.
	...
	function applyOneFlowSvelteSourceMutation(request, source, mutation, sourceFile, sourcePath) {
		...
		var editingNode = (op === "delete" || op === "remove" || op === "setEnabled")
			&& String(mutation.path || "").indexOf("frontAst") === 0;
		var insertionTarget;
		var documentRequest;
		if (op === "move" || inserting || wrapping || replacing || editingNode) {
			documentRequest = Object.assign({}, request, {
				source: source, sourceFile: String(sourceFile.getAbsolutePath()),
				sourcePath: String(sourceFile.getAbsolutePath()), sourceTree: true, includeBindings: false
			});
			var document = describeFrontendDocument(documentRequest);
			var contractRequest = Object.assign({}, mutation, {
				sourcePath: String(sourceFile.getAbsolutePath())
			});
			if (inserting) {
				insertionTarget = flowTreeService().authoringInsertTargetFromTreeRequest(contractRequest,
					document && document.tree, flowTreeServiceEnv());
				mutation = Object.assign({}, mutation, { path: insertionTarget.path, index: insertionTarget.index });
			} else if (wrapping || replacing) {
				contractRequest.paths = wrapping ? mutation.paths || [mutation.from || mutation.source || mutation.path] : [mutation.path];
				insertionTarget = flowTreeService().authoringReplaceSelectionTargetFromTreeRequest(contractRequest,
					document && document.tree, flowTreeServiceEnv());
				mutation = wrapping ? Object.assign({}, mutation, { paths: insertionTarget.paths }) : mutation;
			} else if (editingNode) {
				var selected = flowTreeService().authoringReplaceSelectionTargetFromTreeRequest(
					Object.assign({}, contractRequest, { paths: [mutation.path] }), document && document.tree, flowTreeServiceEnv());
				mutation = Object.assign({}, mutation, { path: selected.paths[0] });
			} else {
				var validated = flowTreeService().authoringMoveFromTreeRequest(contractRequest,
					document && document.tree, flowTreeServiceEnv());
				mutation = Object.assign({}, mutation, { from: validated.from, path: validated.path });
				// The writer must use the exact identity that the common contract validated.
				delete mutation.fromId;
			}
		}
		var resourceRoot = frontendSvelteResourceRoot(request);
		...
		try {
			...
			if (insertionTarget) {
				// Render only a draft, then obtain its real descriptor-backed AST. Never
				// trust clipboard traits/slots as a substitute for the provider contract.
				var proposed = describeFrontendDocument(Object.assign({}, documentRequest, { source: result.source }));
				flowTreeService().authoringProposedSubtreeFromTreeRequest(Object.assign({}, insertionTarget, {
					sourcePath: String(sourceFile.getAbsolutePath())
				}), proposed && proposed.tree, flowTreeServiceEnv());
			}
			return {
				ok: true,
	...
	function frontAstStructuralReplacement(mutation) {
		var path = String(mutation.path || "");
		return (mutation.op === "replace" || mutation.op === "set")
			&& /^frontAst.*\[\d+\]$/.test(path) && path.indexOf(".props") < 0
			&& !frontAstPropertyPayload(mutation.value);
	}
